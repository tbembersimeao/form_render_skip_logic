...
use FormRenderSkipLogic\ExternalModule\ExternalModule;

class Migration {

    protected $module;

    function __construct(ExternalModule $module) {
        $this->module = $module;
    }
     ...
     */
     function getFRSLModuleId() {
       $q = "SELECT external_module_id FROM redcap_external_modules where directory_prefix = '" . $this->module->PREFIX . "'" ;
       $result = $this->query($q);
     ...
     */
     function getV2Settings() {
       ...
       $q = "SELECT project_id, `key`, value FROM redcap_external_module_settings WHERE external_module_id='$module_id' AND `key` IN ('control_field', 'event_name', 'field_name', 'enabled_before_ctrl_field_is_set', 'target_instruments', 'instrument_name', 'control_field_value')";
       $result = $this->query($q);
